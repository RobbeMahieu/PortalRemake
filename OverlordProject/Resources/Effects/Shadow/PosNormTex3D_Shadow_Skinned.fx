...
VS_OUTPUT VS(VS_INPUT input)
{
	...
	for (int i = 0; i < 4; ++i) {
		...
		if (boneIndex >= 0) {
			...
			transformedPos += input.BoneWeights[i] * mul(float4(input.pos, 1.0f), boneMatrix);
			transformedNormal += input.BoneWeights[i] * mul(input.normal, (float3x3)boneMatrix);
		}
	}
...
float EvaluateShadowMap(float4 lpos)
{
	...
	if (lpos.x < -1.0f || lpos.x > 1.0f ||
		...
		lpos.z < 0.0f || lpos.z > 1.0f) return (1 - shadowStrength);
	...
	float shadowFactor = sum / 9.0;


	return  1 - (1 - shadowFactor) * shadowStrength;
}
