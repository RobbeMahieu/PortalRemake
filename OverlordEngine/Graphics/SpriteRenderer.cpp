...
void SpriteRenderer::UpdateBuffer(const SceneContext& sceneContext)
{
	if (!m_pVertexBuffer || m_Sprites.size() > m_BufferSize)
	{
		...
		buffDesc.ByteWidth = UINT(sizeof(VertexSprite) * m_Sprites.size());
	...
	if (m_pVertexBuffer)
	{
		...
		HANDLE_ERROR(sceneContext.d3dContext.pDeviceContext->Map(m_pVertexBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0 , &bufferData));
		// use memcpy to copy all our sprite vertices (m_Sprites) to the mapped resource (D3D11_MAPPED_SUBRESOURCE::pData)
		memcpy(bufferData.pData, m_Sprites.data(), sizeof(VertexSprite) * m_Sprites.size());
