...
GameObject* GameScene::SpawnChild(GameObject* object) {
	m_pToAdd.emplace_back(object);
	object->Initialize(m_SceneContext);
...
void GameScene::DestroyChild(GameObject* object) {
	m_pToDelete.emplace_back(object);
}
...
#pragma warning(pop)
	...
	for (auto& object : m_pToAdd) {
		AddChild(object);
	}
	m_pToAdd.clear();

	// Add delete objects from the scene
	for (auto& object : m_pToDelete) {
...
#pragma region SHADOW PASS
	...
	for (const auto object : m_pChildren) {
		object->RootShadowMapDraw(m_SceneContext);
	}
...
#pragma region POST-PROCESSING PASS
	...
	//No need to swap RenderTargets is there aren't any PP Effects...
		if (m_PostProcessingMaterials.size() > 0)
		{
			...
			RenderTarget* initRT{ m_pGame->GetRenderTarget() };
			RenderTarget* prevRT{ initRT };
			...
			for (PostProcessingMaterial* material : m_PostProcessingMaterials) {
				...
				material->Draw(m_SceneContext, prevRT);
				prevRT = material->GetOutput();
			}
			//		For Each Material
			...
			if (initRT != prevRT) {
				...
				SpriteRenderer::Get()->DrawImmediate(m_SceneContext.d3dContext, prevRT->GetColorShaderResourceView(), XMFLOAT2{ 0,0 });
			}
